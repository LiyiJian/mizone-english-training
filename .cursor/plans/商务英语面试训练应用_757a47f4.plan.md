---
name: 商务英语面试训练应用
overview: 从零构建一个纯静态、离线可用的商务英语面试训练 SPA 网页应用，涵盖听力/写作/口语/测验四大模块，内置 14 天达能脉动 Brand Manager 面试强化内容，使用 localStorage 保存进度。
todos:
  - id: scaffold
    content: 创建项目骨架：index.html + CSS + JS 目录结构
    status: in_progress
  - id: storage
    content: 实现 storage.js（localStorage 封装、导入导出）
    status: pending
  - id: app-router
    content: 实现 app.js（SPA 路由、导航、全局初始化）
    status: pending
  - id: style
    content: 实现 style.css（完整 UI 样式、响应式布局）
    status: pending
  - id: utils
    content: 实现工具模块：timer.js, tts.js, recorder.js, scoring.js, charts.js
    status: pending
  - id: content-data
    content: 编写 14 天完整训练内容数据（content.js + vocabulary.js）
    status: pending
  - id: page-dashboard
    content: 实现进度看板页面（连续天数、得分图表、薄弱点）
    status: pending
  - id: page-today
    content: 实现今日训练页面（流程卡片、计时器）
    status: pending
  - id: page-listening
    content: 实现听力训练页面（TTS播放、听写、关键词、评分）
    status: pending
  - id: page-writing
    content: 实现写作训练页面（模板、改写、结构评分）
    status: pending
  - id: page-speaking
    content: 实现口语训练页面（提纲、录音、自评）
    status: pending
  - id: page-quiz
    content: 实现每日测验页面（混合题型、评分、解析）
    status: pending
  - id: page-custom
    content: 实现我的素材页面（案例/方法论 CRUD）
    status: pending
  - id: page-settings
    content: 实现设置页面（导出/导入/重置）
    status: pending
  - id: integration-test
    content: 全流程集成验证（打开即用、所有模块联通、数据持久化）
    status: pending
isProject: false
---
...
```mermaid
graph TD
    IndexHTML["index.html (SPA 入口)"]
    IndexHTML --> NavBar["顶部导航栏"]
    NavBar --> PageDashboard["进度看板"]
    NavBar --> PageToday["今日训练"]
    NavBar --> PageListening["听力训练"]
    NavBar --> PageWriting["写作训练"]
    NavBar --> PageSpeaking["口语训练"]
    NavBar --> PageQuiz["每日测验"]
    NavBar --> PageCustom["我的素材"]
    NavBar --> PageSettings["设置（导入/导出）"]
```
